...
test('MySQL 5.7 创建只读实例冒烟测试', async () => {
  ...
  try {
    ...
    const mysql57 = page.getByText(/^(?:MySQL\s*)?5\.7$/i)
      .filter({ visible: true })
      .last();
    await expect(mysql57).toBeVisible({ timeout: stepTimeout });
    await mysql57.scrollIntoViewIfNeeded();
    await mysql57.click({ force: true });
    console.log('已第一次选择 MySQL 5.7。');

    // 等待联动更新后再次点击 5.7，确保异步默认值不会切回 8.0。
